...
from tests.factories import NUM_RELATED_OBJS, TokenFactory, get_factory
...
class TestImportCommand(TestCase):
  ...
  def test_import(self, token_decode: MagicMock) -> None:
    ...
    num_requests = (
      sum(
        [ceil(self.num_objs[model] / self.per_request[model]) for model in self.models]
      )
    ...
    for model in self.models:
      ...
      if model is Contact:
        ...
        self.assertTrue(Contact.list_memberships.through.objects.exists())
