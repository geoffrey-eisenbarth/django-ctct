...
type RelatedObjects = tuple[type[Model], list[Model]]
...
class Token(CreatedAtMixin, EndpointMixin, Model):
  ...
  API_JWKS_URL: str = (
    "https://identity.constantcontact.com/oauth2/aus1lm3ry9mF7x2Ja0h8/v1/keys"
  )
  API_SCOPE: str = "+".join(
    [
      "account_read",
...
class SerialModel(Model):
  ...
  API_READONLY_FIELDS: tuple[str, ...] = ("api_id",)
...
class ContactCustomField(SerialModel):
  ...
  API_EDITABLE_FIELDS = ("value",)
  API_READONLY_FIELDS = ("custom_field_id",)
  API_MAX_LENGTH = {
    "value": 255,
  }
  ...
  class Meta:
    ...
    constraints = [
      ...
      #   check=Q(contact__custom_fields__count__lte=ContactRemoteManager.API_MAX_NUM['custom_fields']),  # noqa: E501
...
class CustomField(CreatedAtMixin, UpdatedAtMixin, CTCTEndpointModel):
  ...
  type = models.CharField(
    ...
    default=TYPES[0][0],
    verbose_name=_("Type"),
    help_text=_("Specifies the type of value the custom_field field accepts"),
  )
...
class Contact(CreatedAtMixin, UpdatedAtMixin, CTCTEndpointModel):
  ...
  permission_to_send = models.CharField(
    ...
    help_text=_(
      "Identifies the type of permission that the Constant Contact account has to send email to the contact"  # noqa: E501
    ),
  )
...
class ContactPhoneNumber(CreatedAtMixin, UpdatedAtMixin, CTCTModel):
  ...
  class Meta:
    ...
    constraints = [
      ...
      # models.CheckConstraint(   # TODO: GH #8
      #   check=Q(contact__phone_numbers__count__lte=ContactRemoteManager.API_MAX_NUM['phone_numbers'])  # noqa: E501
...
class ContactStreetAddress(CreatedAtMixin, UpdatedAtMixin, CTCTModel):
  ...
  class Meta:
    ...
    constraints = [
      ...
      # models.CheckConstraint(   # TODO: GH #8
      #   check=Q(contact__street_addresses__count__lte=ContactRemoteManager.API_MAX_NUM['street_addresses']),  # noqa: E501
  ...
  def __str__(self) -> str:
    ...
    address = ", ".join(getattr(self, _) for _ in field_names if getattr(self, _))
...
def is_model(val: type[BaseModel] | Literal["self"] | None) -> TypeGuard[type[Model]]:
